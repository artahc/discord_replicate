...
void main() {
  ...
  late Store<String, User> mockDb;
  late Store<String, User> mockCache;
  ...
  test("Load user from remote source, should be able to parse to User model.", () async {
    ...
    when(() => mockDb.save(expectedResult.uid, expectedResult)).thenAnswer((invocation) => Future.value(null));
    when(() => mockCache.load(userId)).thenAnswer((invocation) => Future.value(null));
    when(() => mockCache.save(expectedResult.uid, expectedResult)).thenAnswer((invocation) => Future.value(null));

    var user = await userRepo.getUserById(userId);

    verify(() => mockDb.load(userId)).called(1);
    verify(() => mockDb.save(expectedResult.uid, expectedResult)).called(1);
    verify(() => mockCache.load(userId)).called(1);
    verify(() => mockCache.save(expectedResult.uid, expectedResult)).called(1);
